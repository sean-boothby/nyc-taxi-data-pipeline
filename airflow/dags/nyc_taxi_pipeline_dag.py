...
with DAG(
    ...
    extract_task = BashOperator(
        ...
        bash_command='python /opt/airflow/extract-pipeline/grab_taxi_data.py'
    )

    # Task 2: Load data into PostgreSQL
    load_postgres_task = BashOperator(
        task_id='load_to_postgres',
        bash_command='python /opt/airflow/staging-pipeline/load_to_postgres.py'
    )

    # Task 3: Clean and transform data
    clean_task = BashOperator(
        task_id='clean_data',
        bash_command='python /opt/airflow/staging-pipeline/data_cleanup.py'
    )

    # Task 4: Upload cleaned data to GCS
    upload_gcs_task = BashOperator(
        task_id='upload_to_gcs',
        bash_command='python /opt/airflow/gcs-pipeline/load_to_gcs.py'
    )

    # Task 5: Load data into BigQuery
    load_bigquery_task = BashOperator(
        task_id='load_to_bigquery',
        bash_command='python /opt/airflow/bigquery-pipeline/load_to_bigquery.py'
    )
